...
Subscribe to `<device-id>/echo` to see all commands processed from any source:
...
2. **Other Channel Commands**: Commands from USB, Serial, or other channels:
   ...
   - Responses go back to their originating channel (non-interfering)
   - Plugin returns `false` to allow normal flow

3. **No Response Interference**: The plugin never prevents responses from being sent back to their originating channels. Commands from USB/Serial receive their responses normally through their original channel.
...
- **Echo messages** with QoS 1: All command logs reliably captured
...
3. **Response Capture**: Commands from other channels (USB, Serial, etc.) are echoed to MQTT, but their responses are sent directly to the originating channel and are not captured by the plugin. This is due to the command processing architecture where plugins are called before responses are generated.
